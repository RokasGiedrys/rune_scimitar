...
export const POST_BY_ID_QUERY =
  ...
  {_id, name, username, image, bio},_createdAt, views, description, image, pitch
}`);

export const POST_VIEWS_QUERY =
  defineQuery(`*[_type == "post" && _id == $id][0]{
  _id, views
}`);
...
export const POSTS_BY_AUTHOR_QUERY =
  defineQuery(`*[_type == "post" && author._ref == $id] | order(_createdAt desc) {
  _id, 
  title, 
  slug,
  _createdAt,
